...
def getconnection(): # mysql connecting function
    ...
    return connection
def view_profile():
    # validation for ..restrict acsses if user session is missing
    ...
    try:
        ...
        if result is not None: # showing the user details
        ...
        if conn: conn.close()
def edit_profile(): #editing the profile
    user = auth.get_current_user()
    if user is None: return
    ...
    try:
        ...
        # if input field is blank restore old values
        if new_name == "": new_name = record[0]
        if new_email == "": 
            new_email = record[1]
        else:
            if "@" not in new_email or "." not in new_email: #check for email matching structural bounds
...
def change_password():
    ...
    try:
        ...
        conn = getconnection()
        cursor = conn.cursor()
        cursor.execute("SELECT password FROM users WHERE id = %s", (user["id"],)) #fetching existing saved password 
        db_password = cursor.fetchone()
        if current_password != db_password[0]: # verifying password
...
def toggle_notifications(): # notifications setting method
    user = auth.get_current_user()
    if user is None: return
    ...
    try:
        conn = getconnection()
        cursor = conn.cursor()
        # tracking toggle matrics atrributes
