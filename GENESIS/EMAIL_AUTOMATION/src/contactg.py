...
# MODULE : Contact Group Management
# DESCRIPTION : This module manages Contact Groups and Contacts.
# FEATURES : 1. Create Group, 2. View Groups, 3. Add Contact, 4. View Contacts,
#            5. Update Contact, 6. Delete Contact, 7. Delete Group
...
def add_contact():
    ...
    try:
        ...
        print("\nAvailable Groups")
        print("----------------")
        cursor.execute("SELECT group_id, group_name FROM contact_groups")
        groups = cursor.fetchall()
        ...
        cursor.execute("""
            INSERT INTO contacts (name, email, group_id)
            VALUES (%s, %s, %s)
        """, (name, email, group_id))
...
def update_contact():
    ...
    try:
        ...
        name = input(f"New Name [{contact[1]}] : ").strip()
        email = input(f"New Email [{contact[2]}] : ").strip()
        ...
        view_groups()
        group_id = input("New Group ID : ").strip()
        if group_id == "":
            group_id = contact[3]

        query = """
        UPDATE contacts
        SET name=%s, email=%s, group_id=%s
        WHERE contact_id=%s
        """
        cursor.execute(query, (name, email, group_id, contact_id))
        conn.commit()
