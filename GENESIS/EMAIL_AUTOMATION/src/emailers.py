...
# DESCRIPTION : This module manages Email Reminders.
# FEATURES : 1. Add Reminder, 2. View Reminder, 3. Update Reminder Status,
#            4. Delete Reminder, 5. Automatic Reminder Notification
...
def add_reminder():
    ...
    try:
        ...
        while True:
            reminder = input("Reminder Date (YYYY-MM-DD HH:MM:SS) : ").strip()
            try:
                reminder_datetime = datetime.strptime(reminder, "%Y-%m-%d %H:%M:%S")
        ...
        query = """
        INSERT INTO email_reminders (recipient_email, subject, message, reminder_date, status)
        VALUES (%s, %s, %s, %s, %s)
        """
        cursor.execute(query, (receiver, subject, message, reminder_datetime, "Pending"))
        conn.commit()
...
def view_reminders():
    ...
    try:
        cursor = conn.cursor(buffered=True)
        query = """
        SELECT id, recipient_email, subject, message, reminder_date, status
        ...
        for row in records:
            ...
            if isinstance(row[4], datetime):
                print("Reminder Date   :", row[4].strftime("%Y-%m-%d %H:%M:%S"))
            else:
...
def update_status():
    ...
    try:
        ...
        cursor.execute("SELECT id FROM email_reminders WHERE id=%s", (reminder_id,))
        ...
        cursor.execute(
            "UPDATE email_reminders SET status=%s WHERE id=%s",
            (status, reminder_id)
        )
        conn.commit()
...
def delete_reminder():
    ...
    try:
        ...
        cursor.execute("DELETE FROM email_reminders WHERE id=%s", (reminder_id,))
        conn.commit()
    ...
    while True:
        ...
        if conn is not None:
            ...
            try:
                ...
                FROM email_reminders
                WHERE reminder_date <= NOW() AND status='Pending'
                """
                cursor.execute(query)
                ...
                for reminder in reminders:
                    ...
                    cursor.execute(
                        "UPDATE email_reminders SET status='Completed' WHERE id=%s",
                        (reminder_id,)
                    )
                    conn.commit()
            except Error as err:
