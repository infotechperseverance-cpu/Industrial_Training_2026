...
LOG_FILE = os.path.join(BASE_DIR, "backup_log.json")
...
# ---------------- Create Backup Entry ----------------
def create_backup_record(filename, category, source, destination, status):
    logs = load()
...
def backup_file():
    ...
    # Automatically create backup folder inside BASE_DIR
    ...
    for item in records:
        if item["filename"] == filename:
            ...
            # --- 🚀 [बदल] 'File_Manager' चा पाथ सुद्धा BASE_DIR ला जोडला ---
            source = os.path.join(BASE_DIR, "File_Manager", category, filename)
            destination = os.path.join(BACKUP_FOLDER, filename)

            if not os.path.exists(source):
                print("Source file not found.")
                create_backup_record(filename, category, source, destination, "Failed")
                return

            if os.path.exists(destination):
                print("File already exists in backup folder.")
                create_backup_record(filename, category, source, destination, "Skipped")
                return

            try:
                shutil.copy2(source, destination)
                create_backup_record(filename, category, source, destination, "Copied")
